...
**Every list is the user's to change.** `core/store.ts` keeps an `Aliases` record — the list name as a phrase writes it, against its words — and **only what they touched is in it**: a key that is absent follows the table into the next release. A key that is present wins outright, **an empty list included**, because emptying one is a thing somebody can mean and falling back to the shipped words would make it the one edit the panel could not carry out. A list they invent is reached by writing its name in a phrase, and is the only kind that can be deleted rather than merely emptied.
...
**Only a list the user made can be renamed or deleted.** A name the table ships is written into the phrases that use it — rename `pronouns` and every phrase saying `{pronouns}` quietly resolves to nothing — so those are shown and not offered, and the edit toggle is disabled until there is a list of their own.
...
**An old profile is carried over once.** This panel was **My details** and wrote a `{ name, contacts }` object under `dwellspeak_profile`; those were always these same lists behind the scenes, so `loadAliases` folds one into aliases the first time it finds no alias store. A backup written before the change carries `profile` instead of `aliases`, and `parseBackup` reads it the same way. Renaming a menu item must not lose somebody their contacts.
