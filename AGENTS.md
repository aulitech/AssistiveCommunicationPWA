...
- `public/` - Served at the site root: PWA manifest, icons, and `sw.js` (the offline service worker)
...
- `vitest.config.ts` - Test configuration, kept separate because `vite.config.ts` is regenerated by Figma Make
