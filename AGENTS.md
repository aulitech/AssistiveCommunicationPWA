...
**The muted grey is held to 7:1.** `--text-muted` carries every sublabel, hint and count in the app, and it was `#888` — 4.7:1 on a cell and **4.3:1 on one being hovered**, under what AA asks for, in the one state a control is in while it is being read. It is `#b0b0b0` now, which clears AAA against every surface in the palette. `src/structure.test.ts` does the arithmetic rather than trusting an eye.
