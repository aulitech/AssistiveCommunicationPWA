...
- `ui/link-input.ts` - `useLinkInput`, the paste and drop handlers a text box needs to turn a link into markdown, and `pasteFromClipboard` behind the **paste control** both boxes carry. Used by the message box and by the phrase editor
