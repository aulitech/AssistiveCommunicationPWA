...
- **The passphrase can be read back off the device that has it**, and so can the ElevenLabs key — `SecretField` in the settings panel: hidden by default, **copyable without ever being shown**, and shown only on a second deliberate act, because this panel spans the viewport and these are set up in rooms with other people in them. It does not re-hide itself on a timer: somebody reading a passphrase out to whoever holds the second device needs it to stay put. The key was write-only until this existed, and ElevenLabs shows one exactly once.
