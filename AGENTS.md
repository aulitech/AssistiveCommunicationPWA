...
- **The strip under the box carries the rest**, which is two things — the category and the voice. Both are triggers onto a full-screen grid, because that is the only shape of "one out of many" a dwell can work. The category was a native `<select>`, whose list the operating system draws outside the page, where nothing can be hovered at all. Its grid ends in a tile that is not a category: **New category…** hands over to the same `CategoryModal` the tabs use, since naming one is a keyboard job.
...
- **Edit mode and auto-speak are exclusive.** They ask opposite things of one dwell on a phrase — one makes it a thing to say this instant, the other a thing to rewrite — so switching either on switches the other off.
...
Five numeric values, a voice, and a linked account. Three things about putting them back:
