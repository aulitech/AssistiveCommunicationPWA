...
**The panel folds, and each list arranges.** One list open at a time, the first open on arrival, and opening one scrolls it to the top of the pane — the same bargain the guide's headings strike, and for the same reason: `bodyparts` alone is fifty words to scroll past to reach the list you came for. A closed list is **unmounted, not hidden**, or a screen reader reads out three hundred words nobody asked for. The open one draws its words as a **grid**, and carries the two arrangements the category tabs offer — A–Z, or the order the words were put in — plus an arrange mode out of `ui/reorder`. Three things about it:
