...
- **A second strip rides the box's lower border**, centred on it exactly as the modes ride the upper one, and carrying the two things a phrase has besides its words — the category and the voice. Both are triggers onto a full-screen grid, because that is the only shape of "one out of many" a dwell can work. The category was a native `<select>`, whose list the operating system draws outside the page, where nothing can be hovered at all. Its grid ends in a tile that is not a category: **New category…** hands over to the same `CategoryModal` the tabs use, since naming one is a keyboard job. `.edit-bar` `bottom` has to match `.topbar` padding-bottom, and `.app.edit-mode .topbar` has to add half the strip's height to that padding — the same pairing `.topbar-modes` has above, and the same failure if it drifts.
...
Five numeric values, a voice, and a linked account. **`autoSpeak` ships on** — see the mode ring under *Editing a phrase* — which is why every test that composes a phrase into the box has to say `{ autoSpeak: false }` for itself, and why `readSettings` falls its missing case back to the default rather than to `false`. Three things about putting them back:
