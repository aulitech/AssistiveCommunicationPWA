...
- **The API key is never in a backup.** It lives under its own storage key, outside the three things `buildBackup` is built from. A backup is made to be shared, and the key in one hands over the account. `tests/core/backup.test.ts` holds it to that, and `src/legal.ts` says so to the user.
