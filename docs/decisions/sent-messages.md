...
Every message spoken or copied is kept, newest first, and shown under a **Sent** tab pinned to the left of **Library**. Selecting one puts it back in the message box; in edit mode it comes into the box to be *kept* as a real phrase, and the bin *forgets* it.

- **Sent is not a category.** Its filter id is `SENT_FILTER`, which begins with a space so no real category — names are trimmed — can ever collide with it. Both it and Library carry `fixed: true` in the `categories` prop, which is how `FilterBar` knows a tab cannot be renamed, dragged or reordered.
