...
- **`vite.config.ts` is generated by Figma Make and gets overwritten.** Test config lives in `vitest.config.ts` for exactly this reason. Do not add settings you need to keep to `vite.config.ts`.
